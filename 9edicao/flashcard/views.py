...
def deletar_flashcard(request, id):
    ...
    if request.user != flashcard.user:
        return HttpResponseForbidden('Você não possui permissão de excuir esse flashcard')
...
def iniciar_desafio(request):
    if request.method == 'GET':
        ...
        flashcards = request.POST.getlist('flascards')
...
def relatorio(request, id):
    ...
    errados = desafio.flashcard.filter(acertou=False).count()
    ...
    for categoria in categorias:
        data.append(desafio.flashcard.filter(flashcard__categoria=categoria).filter(acertou=True).count())
